...
public class UserService {
  ...
  public String signUp(UserDTO userDTO) {
    ...
    boolean isAllowed =
        allowedUserRepository.findByNameAndRegNo(userDTO.name(), encryptedRegNo).isPresent();
    if (!isAllowed) {
  ...
  public String login(UserDTO userDTO) {
    UserEntity userEntity =
        userRepository
            .findByUserId(userDTO.userId())
            .orElseThrow(
                () -> new CustomServiceException(HttpStatus.UNAUTHORIZED, "회원이 존재하지 않습니다."));
  ...
  public String deleteUser(String userId) {
    UserEntity userEntity =
        userRepository
            .findById(userId)
            .orElseThrow(
                () -> new CustomServiceException(HttpStatus.NOT_FOUND, "삭제할 사용자를 찾을 수 없습니다."));
